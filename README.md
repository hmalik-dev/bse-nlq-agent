...
it. `Ctrl+C` stops everything. Without a key it prints a line and uses the
...
- **Five statuses, always HTTP 200:** `answered`, `empty` (no rows), `unanswerable`
  ...
  code the UI turns into one sentence). The only HTTP error is a 422 for a bad body.
...
Tickets live in Linear, project [BSE NLQ](https://linear.app/vendor-marketplace/project/bse-nlq-273533ecd95d).
