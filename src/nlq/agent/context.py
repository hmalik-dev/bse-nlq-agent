...
- A "how many" or "how much" question about one club's home games, or another
  short run of events, over a month or less returns one row per event (name,
  event_date and the measure) largest first, with no LIMIT, so the
  rows show where the total comes from. A count over a season, a year, a whole
  category or all events stays one total row.
