    ...
    for name in ("first.db", "second.db"):
        ...
        try:
            fingerprints.append(conn.execute(
                "SELECT COUNT(*), ROUND(SUM(price), 2) FROM tickets"
            ).fetchone())
        finally:
            conn.close()
